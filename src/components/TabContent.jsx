...
function PendingTab({ dept, isAdmin }) {
  ...
  useEffect(() => {
    ...
    if (changed) updateDoc(doc(db, 'organizations', orgId, 'departments', dept.id), { pendingTasks: validTasks });
  ...
  const validateTask = async (taskId) => {
    ...
    if (code === ADMIN_PIN) {
      ...
      await updateDoc(doc(db, 'organizations', orgId, 'departments', dept.id), { pendingTasks: updated });
  ...
  const removeTask = async (idx) => {
    if (confirm("Supprimer définitivement cette remarque ?")) {
       await updateDoc(doc(db, 'organizations', orgId, 'departments', dept.id), { pendingTasks: (dept.pendingTasks || []).filter((_, i) => i !== idx) });
    }
  };
...
function ChecklistTab({ dept, isAdmin }) {
  ...
  const orgId = org?.id;
  const [input, setInput] = useState('');
  const addTask = async () => { const v = input.trim(); if (!v) return; await updateDoc(doc(db, 'organizations', orgId, 'departments', dept.id), { checklist: [...(dept.checklist || []), { id: Date.now(), text: v, done: false, createdAt: Date.now() }] }); setInput(''); };
  const toggle = async (idx) => { const nl = [...(dept.checklist || [])]; nl[idx] = { ...nl[idx], done: !nl[idx].done, checkedAt: !nl[idx].done ? Date.now() : null }; await updateDoc(doc(db, 'organizations', orgId, 'departments', dept.id), { checklist: nl }); };
  const remove = async (idx) => { await updateDoc(doc(db, 'organizations', orgId, 'departments', dept.id), { checklist: dept.checklist.filter((_, i) => i !== idx) }); };
...
function RemindersTab({ dept, isAdmin }) {
  ...
  const add = async () => { if (!text.trim() || !date) return; await updateDoc(doc(db, 'organizations', orgId, 'departments', dept.id), { reminders: [...(dept.reminders || []), { id: Date.now(), text: text.trim(), date }] }); setText(''); setDate(''); };
  const remove = async (idx) => { await updateDoc(doc(db, 'organizations', orgId, 'departments', dept.id), { reminders: dept.reminders.filter((_, i) => i !== idx) }); };
...
function TrashView({ departments, isAdmin }) {
  ...
  useEffect(() => {
    if(isAdmin) {
      deleted.forEach(d => { if (Date.now() - d.deletedAt > 15 * 24 * 60 * 60 * 1000) deleteDoc(doc(db, 'organizations', orgId, 'departments', d.id)); });
    }
  }, []);
  return (
    <div className="main-body">
      ...
      <div className="trash-wrap">
        {deleted.map(d => (
          <div key={d.id} className="trash-item"><span style={{ fontWeight: 600 }}>{d.name}</span>{isAdmin && <button className="btn-restore" onClick={() => updateDoc(doc(db, 'organizations', orgId, 'departments', d.id), { deletedAt: null })}>Restaurer</button>}</div>
        ))}
      </div>
    </div>
  );
